    ...
    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-4 mb-6 shadow-sm flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">รับข้อความทันทีเมื่อแพ็กเกจต่ออายุสำเร็จ</p>
        </div>
      </div>
      <button 
        onClick={subscribeUser}
        disabled={isLoading}
        className="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-xl shadow-sm hover:bg-blue-700 transition-colors disabled:opacity-50"
      >
        {isLoading ? 'กำลังเปิด...' : 'เปิดใช้งาน'}
      </button>
    </div>
  );
}
